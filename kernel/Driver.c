...
NTSTATUS IoControl(PDEVICE_OBJECT DeviceObject, PIRP irp) {
    ...
    else if (Controlcode == IO_REQUEST_PROCESS_PROTECT)
        Status = ProtectPrcoess(irp);
    else if (Controlcode == IO_REQUEST_PROCESS_UNPROTECT)
        Status = UnProtectPrcoess(irp);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDriverObject, PUNICODE_STRING pRegistryPath) {
    ...
    RtlInitUnicodeString(&dev, L"\\Device\\userprotect");
    RtlInitUnicodeString(&dos, L"\\??\\userprotect");
    ...
    if (GetOffset(PsGetCurrentProcess())) {
        ...
        DbgPrintEx(0, 0, "[*] Failed Get EPROCESS Offsets\n");
    } 
